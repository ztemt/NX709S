...
#define HAPTIC_HV_DRIVER_VERSION	"v0.5.2.1"
...
struct dev_pm_qos_request aw_pm_qos_req_vb;
...
static int rtp_osc_cali(struct aw_haptic *aw_haptic)
{
	...
	dev_pm_qos_add_request(aw_haptic->dev,&aw_pm_qos_req_vb, DEV_PM_QOS_MAX_FREQUENCY,AW_PM_QOS_VALUE_VB);
	while (1) {
	...
	dev_pm_qos_remove_request(&aw_pm_qos_req_vb);
...
static void rtp_play(struct aw_haptic *aw_haptic)
{
	...
	//aw_dev_info(aw_haptic->dev, "%s: enter\n", __func__);
	dev_pm_qos_add_request(aw_haptic->dev,&aw_pm_qos_req_vb, DEV_PM_QOS_MAX_FREQUENCY,AW_PM_QOS_VALUE_VB);
	aw_haptic->rtp_cnt = 0;
	mutex_lock(&aw_haptic->rtp_lock);
...
//add by joes
			//do_gettimeofday(&aw_haptic->audio_trig);
			aw_haptic->audio_trig =  ktime_get();
			mutex_lock(&aw_haptic->lock);
		    aw_haptic->touch_audio_gap = ktime_to_us(ktime_sub(aw_haptic->audio_trig, aw_haptic->touch_press));
			
			aw_info("%s: touch_audio_gap = %ldus \n", __func__,aw_haptic->touch_audio_gap);
			
            aw_info("touch=%d, gap_min=%d, gap_max=%d, touch_enable=%d, audio_trig=%ld, touch_press=%ld",
		    aw_haptic->touch,
		    aw_haptic->gap_min,
		    aw_haptic->gap_max,
		    aw_haptic->touch_enable,
		    aw_haptic->audio_trig,
		    aw_haptic->touch_press);
			if (((aw_haptic->touch_audio_gap >= aw_haptic->gap_min) && (aw_haptic->touch >= 1) && (aw_haptic->touch_enable == AW8697_HAPTIC_TOUCH_ENABLE))
					|| ((aw_haptic->touch_audio_gap <= aw_haptic->gap_max) && (aw_haptic->touch < 1) && (aw_haptic->touch_enable == AW8697_HAPTIC_TOUCH_ENABLE))
					||(aw_haptic->touch_enable == AW8697_HAPTIC_TOUCH_DISABLE))
    {   
			//end by joes
			...
			aw_haptic->func->set_wav_seq(aw_haptic, 0x00, aw_haptic->haptic_audio.ctr.wavseq);
			aw_haptic->func->set_wav_seq(aw_haptic, 0x01, 0x00);

			//add by joes
			if (aw_haptic->haptic_audio.ctr.loop == 15 )
			{
				aw_haptic->haptic_audio.ctr.loop = 14;
			}
			//end by joes

			aw_haptic->func->set_wav_loop(aw_haptic, 0x00,aw_haptic->haptic_audio.ctr.loop);
...
#endif
//vincent surport touch add by joes
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	aw_haptic->touch = val;
	//do_gettimeofday(&aw_haptic->touch_press);
	aw_haptic->touch_press = ktime_get();
	//get time
	mutex_unlock(&aw_haptic->lock);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(2 == sscanf(buf, "%x %x", &databuf[0], &databuf[1])) {
		...
		mutex_lock(&aw_haptic->lock);
		aw_haptic->gap_min = databuf[0];
		aw_haptic->gap_max = databuf[1];
		mutex_unlock(&aw_haptic->lock);
	}
...
static int vibrator_init(struct aw_haptic *aw_haptic)
{
	...
	aw_haptic->vib_dev.name = "vibrator_y";
...
static void haptic_init(struct aw_haptic *aw_haptic)
{
    ...
    aw_haptic->gap_min = 10000;
	aw_haptic->gap_max = 500000;
