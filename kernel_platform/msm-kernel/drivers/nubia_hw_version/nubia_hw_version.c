...
static ssize_t nubia_hw_pcb_ext_show(struct kobject *kobj,
		struct kobj_attribute *attr, char *buf)
{
#if IS_ENABLED(CONFIG_NUBIA_PRODUCT_NX709J)
    char pcb_ext[3][12]={"NX709J_V1A","NULL","NX709J_V1A"};
#else
    char pcb_ext[3][12]={"NX679J_V1A","NULL","NX679J_V1A"};
#endif
	...
	return snprintf(buf, CONFIG_STRING_LENGTH + 3, "%s", buf);
}
