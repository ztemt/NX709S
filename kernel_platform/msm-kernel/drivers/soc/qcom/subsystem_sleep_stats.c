...
//#ifdef CONFIG_NUBIA_POWER_DEBUG
...
//#ifdef CONFIG_NUBIA_POWER_DEBUG
#define POWER_MONITOR_PERIOD_MS	300000
...
static uint32_t pre_count[12];
static int init_powerdebug=0;
static struct sleep_stats_data *pg_drvdata;


static int power_subsystem_sleep_stats(unsigned int nid,unsigned int pid, unsigned int idx,const char *name)
{
	struct sleep_stats *subsystem_stats_data;
	struct sleep_stats *stats;

	stats = kzalloc(sizeof(struct sleep_stats), GFP_KERNEL);
	if (!stats) goto out_free;

	if (pid == SUBSYSTEM_STATS_OTHERS_NUM){
		memcpy_fromio(stats, pg_drvdata->reg[idx], sizeof(*stats));
    }else{
	   subsystem_stats_data = qcom_smem_get(pid, idx, NULL);
	   if (IS_ERR(subsystem_stats_data))
		  goto out_free;
	...
	if(nid < 12){
	   pr_err("[pmdb] %s diff:0x%x Count:0x%x;Entered:0x%llx;Exited:0x%llx;Duration:0x%llx \n",
			name, stats->count-pre_count[nid],stats->count,
			stats->last_entered_at, stats->last_exited_at,
			stats->accumulated);
	
	   pre_count[nid]=stats->count;
	}
...
static void power_debug_work_func(struct work_struct *work)
{
	...
	pid = PID_OTHERS;
	smem_item = AOSD;
	power_subsystem_sleep_stats(AOSD_NID,pid, smem_item,"AOSD");

	pid = PID_OTHERS;
	smem_item = CXSD;
	power_subsystem_sleep_stats(CXSD_NID,pid, smem_item,"CXSD");

	pid = PID_MPSS;
	smem_item = MPSS;
	power_subsystem_sleep_stats(MPSS_NID,pid, smem_item,"modem");

	pid = PID_ADSP;
	smem_item = ADSP;
	power_subsystem_sleep_stats(ADSP_NID,pid, smem_item,"adsp");

	pid = PID_ADSP;
	smem_item = SLPI_ISLAND;
	power_subsystem_sleep_stats(ADSP_ISLAND_NID,pid, smem_item,"adsp_island");

	pid = PID_CDSP;
	smem_item = CDSP;
	power_subsystem_sleep_stats(CDSP_NID,pid, smem_item,"cdsp");

	pid = PID_SLPI;
	smem_item = SLPI;
	power_subsystem_sleep_stats(SLPI_NID,pid, smem_item,"slpi");

	pid = PID_SLPI;
	smem_item = SLPI_ISLAND;
	power_subsystem_sleep_stats(SLPI_ISLAND_NID,pid, smem_item,"slpi_island");
...
static void power_debug_work_control(int on){
	if(1==on){
		init_powerdebug=1;
...
//#ifdef CONFIG_NUBIA_POWER_DEBUG
    if(init_powerdebug < 1){
	   pg_drvdata=stats_data;
	   power_debug_work_control(1);
	}
//#endif
