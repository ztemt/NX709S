...
#define ALS_CAL_FILE_PATH        "/persist/sensors/calib/als_cal_data"
#define ALS_FAC_CAL_PATH        "/persist/sensors/calib/als_fac_cal"

#define PS_NAME "proximity"
#define PS_CAL_FILE_PATH      "/persist/sensors/calib/xtalk_cal"
...
#define STK_CAL_READ	0xaa55
#define STK_CAL_WRITE	0x55aa
...
//  input_report_abs(stk_data->ps_input_dev, ABS_DISTANCE, nf);
	input_report_rel(stk_data->ps_input_dev, REL_RZ, nf? 5:1);
	...
	struct device_attribute *attr, char *buf)
{
	...
	if(reading < 0){
		mutex_unlock(&stk_data->io_lock);
		SENSOR_LOG_ERROR("stk3x3x_get_als_reading err, ret=%d\n", reading);
		return -1;
	}
...
#ifdef CONFIG_SENSOR_STK_GKI
static ssize_t nb_stk3x_cal_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	//struct stk3x3x_data *stk_data = container_of(file->private_data, struct stk3x3x_data, miscdev);
	
	//SENSOR_LOG_INFO("%s enter.miscdev:%s.\n", __func__, stk_data->miscdev.name);
	
	mutex_lock(&stk_cal.cal_mutex);
	if(stk_cal.cal_buf.o_flag != 0){
		if (copy_to_user(buf, &stk_cal.cal_buf, sizeof(ST_COM_PAC))) {
			SENSOR_LOG_INFO("%s send cal package to user failed.\n", __func__);
			mutex_unlock(&stk_cal.cal_mutex);
			return -EFAULT;
		}
	}
	SENSOR_LOG_INFO("%s:kernel send cmd:%x to user.\n", __func__, stk_cal.cal_buf.o_flag);
	stk_cal.cal_buf.o_flag = 0;
	mutex_unlock(&stk_cal.cal_mutex);

	return sizeof(ST_COM_PAC);
}
static ssize_t nb_stk3x_cal_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
	//struct stk3x3x_data *stk_data = container_of(file->private_data, struct stk3x3x_data, miscdev);
	int ret = 0;
	...
	mutex_lock(&stk_cal.cal_mutex);
	ret = copy_from_user(&stk_cal.cal_buf, buf, sizeof(ST_COM_PAC));
	if (ret) {
		mutex_unlock(&stk_cal.cal_mutex);
		SENSOR_LOG_INFO("%s copy the cal data_len=%d  from_user error.\n", __func__, count);
		return -EFAULT;
	}
	stk_cal.cal_buf.o_flag = 0;
	mutex_unlock(&stk_cal.cal_mutex);
	SENSOR_LOG_INFO("%s: copy from user len:%d.\n", __func__, count);

	complete(&stk_cal.m);

	return sizeof(ST_COM_PAC);
}
static unsigned int nb_stk3x_cal_poll(struct file *file, poll_table *wait)
{
	int ret = 0;
	//struct stk3x3x_data *stk_data = container_of(file->private_data, struct stk3x3x_data, miscdev);
	...
	poll_wait(file, &stk_cal.read_wait, wait);
	mutex_lock(&stk_cal.cal_mutex);
	if (stk_cal.cal_buf.o_flag != 0)
...
static int nb_stk3x_cal_open(struct inode *inode, struct file *file)
{
	//struct stk3x3x_data *stk_data = container_of(file->private_data, struct stk3x3x_data, miscdev);
...
static int nb_stk3x_cal_close(struct inode *inode, struct file *file)
{
	//struct stk3x3x_data *stk_data = container_of(file->private_data, struct stk3x3x_data, miscdev);
...
static const struct file_operations nb_stk3x_cal_fops = {
	.owner		= THIS_MODULE,
	.read		= nb_stk3x_cal_read,
	.write		= nb_stk3x_cal_write,
	.poll		= nb_stk3x_cal_poll,
	.open		= nb_stk3x_cal_open,
	.release	= nb_stk3x_cal_close,
};
#endif
...
#ifdef CONFIG_SENSOR_STK_GKI
	init_waitqueue_head(&stk_cal.read_wait);
	init_completion(&stk_cal.m);
	mutex_init(&stk_cal.cal_mutex);
	memset(&stk_cal.cal_buf, 0, sizeof(stk_cal.cal_buf));
	stk_data->miscdev.minor = MISC_DYNAMIC_MINOR;
	stk_data->miscdev.name = "stk_sns";
	stk_data->miscdev.fops = &nb_stk3x_cal_fops;
	stk_data->miscdev.parent = &client->dev;
	err = misc_register(&stk_data->miscdev);
	if (err < 0) {
		SENSOR_LOG_INFO(" Faild to register miscdev for calibrate!");
	}else{
		SENSOR_LOG_INFO(" success to register miscdev for stk calibrate!");
	}
#endif
...
#endif
#ifdef CONFIG_SENSOR_STK_GKI
	misc_deregister(&stk_data->miscdev);
#endif
#ifdef STK_QUALCOMM_POWER_CTRL
	stk3x3x_power_init(stk_data, false);
#endif
...
int get_cal_file_type(char *file_path)
{
	if(!strcmp(ALS_CAL_FILE_PATH, file_path))
		return 1;
	if(!strcmp(ALS_FAC_CAL_PATH, file_path))
		return 2;
	if(!strcmp(PS_CAL_FILE_PATH, file_path))
		return 3;
...
//#if 1
	...
	mutex_lock(&stk_cal.cal_mutex);
	stk_cal.cal_buf.o_flag = STK_CAL_READ;
	stk_cal.cal_buf.f_type = get_cal_file_type(file_path);
	stk_cal.cal_buf.d_len = count;
	stk_cal.cal_buf.data = 0;
	mutex_unlock(&stk_cal.cal_mutex);
	wake_up_interruptible(&stk_cal.read_wait);

	SENSOR_LOG_ERROR("%s:kernel request read file:%s len:%d.\n", __func__, file_path, count);
	rc = wait_for_completion_timeout(&stk_cal.m, HZ*2);	//timeout 2S
	if(rc == 0){
		mutex_lock(&stk_cal.cal_mutex);
		memset(&stk_cal.cal_buf, 0, sizeof(stk_cal.cal_buf));
		mutex_unlock(&stk_cal.cal_mutex);
		SENSOR_LOG_ERROR("can't get the completion msg!\n");
		return 0;
	}

	SENSOR_LOG_ERROR("%s:kernel success read file:%s len:%d data:%d.\n", __func__, file_path, stk_cal.cal_buf.d_len, stk_cal.cal_buf.data);

	mutex_lock(&stk_cal.cal_mutex);
	memcpy(read_buf, &stk_cal.cal_buf.data, count);
	rc = stk_cal.cal_buf.d_len;
	memset(&stk_cal.cal_buf, 0, sizeof(stk_cal.cal_buf));
	mutex_unlock(&stk_cal.cal_mutex);
...
//#if 1
	...
	mutex_lock(&stk_cal.cal_mutex);
	stk_cal.cal_buf.o_flag = STK_CAL_WRITE;
	stk_cal.cal_buf.f_type = get_cal_file_type(file_path);
	stk_cal.cal_buf.d_len = count;
	memcpy(&stk_cal.cal_buf.data, write_buf, count);
	mutex_unlock(&stk_cal.cal_mutex);
	wake_up_interruptible(&stk_cal.read_wait);

	SENSOR_LOG_ERROR("%s:kernel request wirte file:%s len:%d data:%d.\n", __func__, file_path, count, stk_cal.cal_buf.data);
	rc = wait_for_completion_timeout(&stk_cal.m, HZ*2);	//timeout 2S
	if(rc == 0){
		mutex_lock(&stk_cal.cal_mutex);
		memset(&stk_cal.cal_buf, 0, sizeof(stk_cal.cal_buf));
	...
	SENSOR_LOG_ERROR("%s:kernel success write file:%s len:%d.\n", __func__, file_path, rc);
