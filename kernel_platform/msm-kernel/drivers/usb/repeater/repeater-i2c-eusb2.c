...
//nubia debug
struct eusb2_repeater *usb_eyes_test;
//nubia debug
...
static int eusb2_i2c_read_reg(struct eusb2_repeater *er, u32 reg, u8 *val)
{
	int ret;
...
static ssize_t param_override_seq_show(struct kobject *kobj,
		struct kobj_attribute *attr, char *buf)
{
	...
	if (usb_eyes_test->param_override_seq == NULL)
		return 0;

	ret = eusb2_i2c_read_reg(usb_eyes_test, usb_eyes_test->param_override_seq[1], &reg_val_a);
	ret = eusb2_i2c_read_reg(usb_eyes_test, usb_eyes_test->param_override_seq[3], &reg_val_b);

	return scnprintf(buf, 16, "%x %x\n", reg_val_a, reg_val_b);
}

static ssize_t param_override_seq_store(struct kobject *kobj,
	struct kobj_attribute *attr, const char *buf, size_t size)
{
	...
	for (i = 0; i < 4; i++) {
		usb_eyes_test->param_override_seq[i] = tx_buf[i];
		pr_err("sqc eusb2 param_override_seq_store tx_buf = %d, %d\n", tx_buf[i]);
	}

	return size;
}

static struct kobj_attribute usb_eye_attrs[] = {
	__ATTR(param_override_seq, 0664, param_override_seq_show, param_override_seq_store),
};
//nubia debug
...
//nubia debug
	enhance_kobj = kobject_create_and_add("usb_eyes", kernel_kobj);
	if (!enhance_kobj) {
	...
	for (attr_count = 0; attr_count < ARRAY_SIZE(usb_eye_attrs); attr_count++) {
		ret = sysfs_create_file(enhance_kobj, &usb_eye_attrs[attr_count].attr);
		if (ret < 0) {
...
//nubia debug
	for (--attr_count; attr_count >= 0; attr_count--)
		sysfs_remove_file(enhance_kobj, &usb_eye_attrs[attr_count].attr);
